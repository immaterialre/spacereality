...
* Header Text element
*/
if( !function_exists('pix_get_header_text') ){
...
if( !function_exists('pix_get_header_phone') ){
    function pix_get_header_phone($opts){
        ...
        ?>
        <a data-anim-type="<?php echo esc_attr($animation); ?>" href="tel:<?php echo esc_attr($tel);?>" class="text-sm d-inline-block2 pix-header-text d-inline-flex align-items-center pix-py-5 <?php echo esc_attr( $classes ); ?> mb-0" style="<?php echo esc_attr( $custom ); ?>" ><i class="pixicon-phone text-18 pix-mr-5 pix-header-icon-style"></i> <?php echo esc_html($text);?></a>
        <?php
    }
}
...
if( !function_exists('pix_get_header_search') ){
    function pix_get_header_search($opts){
        ...
        ?>
        <a data-anim-type="<?php echo esc_attr($animation); ?>" href="#" class="btn pix-header-btn btn-link p-0 pix-px-15 pix-search-btn pix-toggle-overlay m-0 <?php echo esc_attr($animation_class); ?> d-inline-flex align-items-center text-<?php echo esc_attr( $color ); ?>" <?php echo esc_attr( $custom ); ?>><i class="pixicon-zoom text-18 pix-header-text font-weight-bold"></i></a>
        <?php
    }
}
...
function pix_get_header_menu($opts){
    extract(shortcode_atts(array(
        ...
        if(!in_array($area, $desktop_areas)){
            ?>
            <button class="navbar-toggler hamburger--spin hamburger small-menu-toggle d-flex d-lg-none" type="button" data-toggle="collapse" data-target="#navbarNav-<?php echo esc_attr( $nav_id ); ?>" aria-controls="navbarNav-<?php echo esc_attr( $nav_id ); ?>" aria-expanded="false" aria-label="Toggle navigation">
    ...
    <div class="<?php echo esc_attr($animation_div_class);?> d-flex align-items-center">
        <div class="d-inline-block <?php echo esc_attr($animation_class);?>" data-anim-type="<?php echo esc_attr($animation);?>" style="<?php echo esc_attr( $max ); ?>">
            ...
            if($themeDefault){
                ...
                if( !empty($area) && ($area == 'm_header' || $area == 'm_stack' || $area == 'm_topbar') && pix_get_option('mobile-logo-img')['url']  ){
                    ?>
                    <a class="navbar-brand" href="<?php echo esc_url( $link ); ?>" rel="home">
